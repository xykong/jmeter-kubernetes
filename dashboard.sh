...
##influxdb_status=`kubectl get po -n $tenant | grep jmeter-influxdb | awk '{print $2}' | grep Running

influxdb_pod=`kubectl get po -n $tenant | grep jmeter-influxdb | awk '{print $1}'`
...
##kubectl cp $working_dir/jmeter-influxdb-datasource.json -n $tenant $grafana_pod:/jmeter-influxdb-datasource.json
